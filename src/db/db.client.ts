...
export async function getDeviceById(deviceId: number) {
    ...
    if (error) {
        console.error('[DB Client] Error fetching device via id: ' + deviceId, error);
        return null;
    }
...
export async function getDiscountByDeviceId(deviceId: number) {
    ...
    if (error) {
        console.error('[DB Client] Error fetching discount via deviceId: ' + deviceId, error);
        return null;
    }
...
export async function getStocksByDeviceId(deviceId: number) {
    ...
    if (error) {
        console.error('[DB Client] Error fetching stocks via deviceId: ' + deviceId, error);
        return null;
    }
...
export async function updateStock(warehouseId: number, updatedStockUnits: number, deviceId: number) {
    ...
    if (error) {
        console.error('[DB Client] Error updating stock for deviceId: ' + deviceId + ' in warehouseId: ' + warehouseId, error);
        return null;
    }

    if (!data || data.length === 0) {
        console.warn('[DB Client] No stock updated for deviceId: ' + deviceId + ' in warehouseId: ' + warehouseId);
        return null;
    }
...
export async function addOrder(totalPrice: number, discount: number, shippingCost: number) {
    const { data, error } = await supabase.from('orders')
        .insert({ totalPrice: totalPrice, discount: discount, shipping: shippingCost })
        .select();
